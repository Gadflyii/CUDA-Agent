...
PRODUCER = "scripts/build_normalized.py@1"
SECRET_PATTERNS = [
...
def authored_source(source_id: str, title: str, relative: str, created_at: str) -> dict[str, Any]:
    path = ROOT / relative
    ...
    return {
        ...
        "notes": "Generated deterministically from reviewed seed annotations and raw manifest references."
    }
...
def main() -> None:
    ...
    episodes_path = write_generated("normalized/episodes.jsonl", jsonl_bytes(sorted(episodes, key=lambda x: x["episode_id"])))

    generated_paths: list[tuple[str, str, list[str], str]] = [
        ("normalized/episodes.jsonl", "canonical episodes", sorted({ref for ep in episodes for ref in ep["provenance"]["snapshot_refs"]}), "normalized-episodes-v1")
    ]
    for card in architecture["cards"]:
        ...
        generated_paths.append((relative, f"{card['architecture_id']} architecture card", refs, f"architecture-card-{card['architecture_id']}"))
    ...
    generated_paths.append(("normalized/cross_sm_deltas.jsonl", "cross-SM delta records", delta_refs, "cross-sm-deltas-v1"))
    ...
    manifests = [
        ...
        for relative, title, refs, source_id in generated_paths
    ]
    write_generated("normalized/manifest.jsonl", jsonl_bytes(sorted(manifests, key=lambda x: x["record_id"])))
    upsert_jsonl(
        "sources/registry.jsonl",
        [authored_source(source_id, title, relative, built_at) for relative, title, _, source_id in generated_paths],
        "source_id",
    )
    print(f"built {len(episodes)} canonical episodes, {len(architecture['cards'])} cards, and {len(architecture['deltas'])} deltas")
