...
GInfer is a fork of Neroued/ninfer. It loads the same `.ginfer` artifacts. The C++ product
identity is GInfer (`ginfer::`, `include/ginfer/`, binaries `ginfer` / `ginfer-serve`).

GInfer is a from-scratch C++/CUDA inference engine for maximum single-GPU inference performance on
a small set of explicitly registered checkpoint artifacts. The supported identities are
`qwen3.6-27b/groupwise-int`, `qwen3.6-27b/nvfp4`,
`qwen3.8-27b/groupwise-int-dflash2-q4`, `qwen3.8-27b/groupwise-int-dflash2-w8`,
`qwen3.8-27b/nvfp4`, `qwen3.8-27b/nvfp4-dflash2-q4`,
`qwen3.8-27b/nvfp4-dflash2-w8`, `qwen3.6-35b-a3b/groupwise-int`,
`muse-glimmer-30b/groupwise-int`, `muse-glimmer-30b/groupwise-int-dflash-q4`, and
`muse-glimmer-30b/nvfp4`. The current
implementation defaults to a `sm_120a` image and is tuned and measured on NVIDIA GeForce RTX 5090;
launch occupancy is derived from the live SM count, and Ampere (`sm_86`) / Ada (`sm_89`) are
separate compile images. The Qwen identities execute Text, image/video Vision, MTP, prefix reuse,
CLI, OpenAI/Anthropic serving, and measurement through the same public `.ginfer` Engine route; the
35B-A3B target additionally supports text-only DFlash. Muse Glimmer is a separate text-only target
with autoregressive decode, optional DFlash2, compatible token-prefix reuse, CLI, and the same
serving protocols; it rejects Vision and MTP. Qwen3.8 and Muse native-Q4 DFlash2 are qualified on
`sm_120a` and physical `sm_86`. Their other Ampere storage profiles and all `sm_89` DFlash2 routes
remain admitted for real-device qualification and tuning; successful cross-compilation does not
establish qualification. The 35B-A3B DFlash route is likewise
`sm_120a`-only; its non-DFlash groupwise routes remain available in the qualified Ampere and Ada
images. NVFP4 identities require Blackwell NVFP4 instructions and remain `sm_120a`-only.
...
Muse Glimmer is not a Qwen3.6-family Variant. Its package owns its fixed text frontend and ATEM
output semantics, artifact binder, immutable model and DFlash2 views, sequence planning, Program,
prefix transaction, workspace composition, and CUDA Graph policy. It reuses semantically closed
Ops from `src/ops` without importing a Qwen target schedule or mutable target state.
...
- `docs/cli.md`: CLI input, output, sampling, MTP/DFlash, and runtime options;
...
- `docs/maintainer/muse-glimmer-30b.md`: Muse model/artifact facts, text/DFlash execution,
  prefix semantics, and hardware qualification;
...
- `docs/maintainer/kernel-optimization-campaign.md`: isolated worktree preparation, candidate
  accounting, acceptance gates, stop conditions, evidence retention, and GPU/model handoff;
- `include/ginfer/engine.h` and `include/ginfer/types.h`: in-tree C++ product interface.
...
- `.ginfer` is the only C++ product artifact. Do not add `.qus` fallback, extension detection,
  compatibility shims, or a second product lane.
- `include/ginfer/engine.h` and `include/ginfer/types.h` are the opaque Engine interface used by
  in-tree applications and owning host values. GInfer does not currently install or export a C++
  SDK. `include/ginfer/ops/` contains repository-internal semantic Op contracts.
...
- `src/artifact` owns generic `.ginfer` framing, descriptors, binding primitives, and
...
- `src/targets/qwen3_6_27b` and `src/targets/qwen3_6_35b_a3b` own their registered checkpoint
  identities, storage profiles, binder,
  ...
  `src/ops`.
- `src/targets/muse_glimmer_30b` owns the registered Muse identity and storage contract, text-only
  ATEM frontend, binder and conditional DFlash2 residency, `LoadedModel`, sequence planning,
  Program/Text/DFlash schedules, exact token-prefix retention, workspace/state transactions, and
  its graph policy. Semantically closed kernels remain under `src/ops`.
...
numerical kernel/model correctness, `.ginfer` framing/binding, external schema/report behavior, a
...
| `.ginfer` reader/converter/binder | affected contract tests and a real artifact when semantics require it |
...
| product artifact | `out/qwen3_6_27b.ginfer` |
| conversion report | `out/qwen3_6_27b.ginfer.conversion.json` |
| normal build | `build/` |
...
GINFER_WEIGHTS=out/qwen3_6_27b.ginfer
```
