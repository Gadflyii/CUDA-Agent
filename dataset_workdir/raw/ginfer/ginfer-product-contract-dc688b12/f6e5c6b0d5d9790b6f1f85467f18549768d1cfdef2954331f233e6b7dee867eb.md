...
GInfer is a fork of Neroued/ninfer. It loads the same `.ninfer` artifacts. The C++ product
...
`qwen3.8-27b/nvfp4`, `qwen3.6-35b-a3b/groupwise-int`,
`muse-glimmer-30b/groupwise-int`, and `muse-glimmer-30b/groupwise-int-dflash-q4`. The current
...
CLI, OpenAI/Anthropic serving, and measurement through the same public `.ninfer` Engine route; the
...
serving protocols; it rejects Vision and MTP. Muse is qualified only for `sm_120a` until its AR and
DFlash routes complete real-device qualification on `sm_86` and `sm_89`; successful
cross-compilation does not establish that qualification. The 35B-A3B DFlash route is likewise
...
GINFER_WEIGHTS=out/qwen3_6_27b.ninfer
```
